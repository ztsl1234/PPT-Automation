...
class Q2b:
    ...
    def create_ppt(self,input, output, report_data, chart):
        ...
        data_base=data_base.sort_values(by='MeasureValue', ascending=True) 
        ...
        data_b4_industry['row_num'] = np.arange(len(data_b4_industry))
        ...
        data_after_industry['row_num'] = np.arange(len(data_after_industry))

        #data_b4_gender.sort_values(by='MeasureValue', ascending=True) 
        #data_after_gender.sort_values(by='MeasureValue', ascending=True) 
        data_b4_industry=data_b4_industry.sort_values(by='MeasureValue', ascending=False) 
        data_after_industry=data_after_industry.sort_values(by='MeasureValue', ascending=False) 
        ...
        self.create_slide4(prs,data_excite)
    ...
    def create_slide_layout1(self,prs,data_df1,data_df2):
        ...
        slide_layout_slide = prs.slide_layouts[1]
        ...
        cell2.text_frame.paragraphs[0].font.size = Pt(16)
        cell2.text_frame.paragraphs[0].font.name = "Calibri"
        ...
        cell1.text_frame.paragraphs[0].font.size = Pt(16)
        cell1.text_frame.paragraphs[0].font.name = "Calibri"        
    ...
    def create_slide_layout3(self,prs,data_df1,data_df2,data_df3,data_df4):
        logging.debug(f" ---> data_df3={data_df3}")  
        logging.debug(f" ---> data_df4={data_df4}")  
        ...
        for i,row in data_df3.iterrows(): 
            ...
            cell=table2.cell(index+2,0)
            cell.text=f'<{row["Type"]} split {row["row_num"] + 1}>' 
            cell.text_frame.paragraphs[0].font.size = Pt(16)
            cell.text_frame.paragraphs[0].font.name = "Calibri"                
...
logging.basicConfig(filename='q2b.log', filemode='w', level=logging.DEBUG,format=log_format)
...
(df_list,err_msg)=q2b.load_data(CSV_DIR)

input_file=rf"{Q_DIR}\template.pptx"
output_file="output_data.pptx"

for key, df in df_list.items():
    #logging.debug(f"---> key= {key}")
    #logging.debug(f"---> df= {df}")
    q2b.create_ppt(input_file, output_file,df, "report-image.png")
